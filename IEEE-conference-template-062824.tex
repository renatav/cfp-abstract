...
%to host in a neutral, industry-facing manner. 
...
Coccinelle is a tool widely used to detect and fix issues, as well as perform refactoring, in the Linux kernel code. It allows users to specify patterns of interest in any C code and, optionally, their replacements using the Semantic Patch Language. Unlike traditional patches, Coccinelle's semantic patches can modify hundreds of files across thousands of code sites by abstracting away specific details and variations \cite{coccinelle}.

Given its extensive use, and in collaboration with the 
maintainers of Coccinelle, we have implemented semantic 
patches for detecting atoms. While our primary focus is on 
deployment within the Linux kernel, Coccinelle can be used to 
process any C code. Consequently, the semantic patches we have 
developed for detecting atoms can also be applied to other C 
...
Coccinelle's capabilities. All these semantic patches are 
