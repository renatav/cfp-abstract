...
%Semantic Patch Language (SmPL) for 
a user to specify desired patterns  
and refactoring in C code. 
%It was initially designed to 
%document and automate collateral evolutions in device driver code. 
...
atom parsers using Coccinelle. While our primary focus is the Linux kernel, Coccinelle can be used to 
process any C code. Consequently, the parsers we have 
developed to detect atoms can also be applied to other C 
codebases. We have implemented parsers for 14 
out of 15 identified atoms, excluding only one that is beyond 
Coccinelle's capabilities. All these parsers are available in our GitHub repository \cite{githubcocci}. 
